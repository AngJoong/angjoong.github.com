...
1. 디폴트 신호 처리기: 모든 신호마다 커널에 의해 처리
2. 사용자 정의 신호 처리기: 디폴트 신호 처리 대체

### 4.3.1 다중 스레드에서의 신호 전달  
신호를 전달하는 방법은 신호의 유형에 따르 다르다.
...
**동기식 스레드는 반드시 그 신호를 야기한 스레드에게 전달되어야 한다.** 반면, 비동기식 신호의 경우에는 명확하지 않다. 보통 신호는 한번만 처리되어야 하기 때문에 신호를 받겠다고 지정한 스레드들중 **첫번째 스레드에게만 신호가 전달되고 처리된다.** (ctr + c)와 같이 그 프로세스 내 **모든 스레드에게 전달되는 신호** 또한 존재한다.  

## 4.4 스레드 풀 (Thread pools)
다중 스레드에는 여러가지 문제가 존재한다. 첫 번째는 **매 요청 마다 스레드 생성에 시간이 든다.** 두 번째는 **스레드 최대 생성 갯수에 제한이있다.** 스레드를 무한정 만들면 언젠가 CPU 시간, 메모리 공간과 같은 자원이 고갈된다. 이러한 문제를  해결해 줄 수 있는 방법 중 하나가 스레드 풀(pool)이다.  

스레드 풀의 기본 아이디어는 **프로세스를 시작할 때 일정 수의 스레드들을 미리 만들어 두고 관리하는 것이다.** 그러다 요청이 발생하면 풀의 한 스레드를 요청에 할당한다. 요청이 완료되면 스레드는 다시 풀에 들어오고 다음 작업을 기다린다. 스레드 풀은 몇가지 장점을 갖는다.  

1. 새 스레드 생성에 대한 오버헤드가 발생하지 않는다.
2. 스레드 수에 제한을 둬, 과도한 자원 사용을 방지한다.  

## 4.5 스레드별 데이터 (Thread-Specific Data)
한 프로세스에 속한 스레드들은 그 프로세스의 데이터를 대부분 공유한다. 반대로 **각 스레드가 자기만 액세스할 수 있는 데이터** 가 있는데 이 데이터를 스레드별 데이터라고 부른다. 예를 들어 각 트랜잭션을 독립된 스레드가 처리한다고 가정할때, 각 스레드 마다 고유한 트랜잭션 식별자를 연관시키기 위해 스레드별 데이터가 필요하다.  

## 4.6 스케줄러 액티베이션 (Scheduler Activations)
다-대-다 및 두 수준 모델에서 최고의 성능을 보장하기 위해 사용자와 커널간 통신이 고려되어야 한다.  

많은 시스템들은 **사용자와 커널 스레드 사이에 경량 프로세스(LWP)라는 중간 자료 구조를 둔다.** 물리처기에서 스케줄하는 대상은 커널 스레드 이다. 각 LWP는 커널 스레드에 종속되며 커널 스레드가 봉쇄되면 LWP가 봉쇄되고 이에 연관된 사용자 스레드도 봉쇄된다.

<br/>
**&lt;경량 프로세스 - LWP>**  
<img class="aligncenter" src="https://www.cs.uic.edu/~jbell/CourseNotes/OperatingSystems/images/Chapter4/4_13_Lightweight.jpg">
<br/>
<br/>

CPU 중심의 어플리케이션이라면 한순간에 오직 하나의 스레드만이이 실행될 수 있어 LWP는 하나면 충분하다. 반면 입출력 중심의 어플리케이션이라면 봉쇄형 시스템 호출마다 하나의 LWP가 필요하다. 예를 들어 서로 다른 5개의 파일 읽기 가 필요할때, 프로세스가 4개의 LWP를 가지고 있다면 다섯번째 요청은 하나의 LWP라도 커널에서 복귀할 떄까지 기다려야 한다.  

### 4.6.1 스케줄러 액티베이션
사용자 스레드와 커널 스레드 간의 통신 방법.  

1. 커널은 어플리케이션에게 가상처리기(LWP) 집합을 제공하고 어플리케이션은 사용자 스레드를 가용한 가상처리기로 스케줄한다.
2. 커널이 어플리케이션에게 특정 사건을 알리는 **업콜** 은 가상 처리기상에서 실행되는 업콜 처리기에의해 처리된다.
3. 응용 스레드가 봉쇄되려고 하면 업콜이 발생한다.
4. 커널은 스레드가 봉쇄될거라는 사실과 그 스레드의 식별자를 알려 주는 업콜을 한다.
5. 커널은 새로운 가상 처리기를 어플리케이션에 할당한다.
6. 어플리케이션은 새로운 가상 처리기 상에서 업콜 처리기를 실행하고, 이 업콜 처리기는 봉쇄 스레드의 상태를 저장하고, 이 스레드가 실행 중이던 가상 처리기를 반환하다.
7. 업콜 처리기는 새로운 가상 처리기에서 실행 가능한 다른 스레드를 스케줄 한다.
8. 봉쇄되었던 스레드가 재실행이 가능해지면 재실행 업콜을 어플리케이션에 보낸다.
9. 이 사건을 처리하기 위해 커널은 새로운 가상 처리기를 할당하거나 사용자 스레드 하나를 선점하여 그 처리기에서 업콜 처리기를 실행한다. 
